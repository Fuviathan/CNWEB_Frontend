...
import { getProductByBrand, getProductByCategory, getSingleProduct } from '@/state/Products/Action';
...
const UpdateProduct = (props) => {
    ...
    if (categoryList && brandList && props.open) return (
        <div className='absolute w-3/5 px-10 py-5 mt-4 overflow-y-auto -translate-x-1/2 -translate-y-1/2 bg-white min-h-fit h-3/4 min-w-fit top-1/2 left-1/2 rounded-xl'>
            <h3 className="mb-4 text-xl font-semibold tracking-wide">Sửa thông tin sản phẩm</h3>
            <form onSubmit={handleSubmit(onSubmit)}>
                ...
                <textarea className='w-full min-h-[100px] focus:opacity-100 overflow-y-auto p-2 mt-2 border border-gray-300 rounded-lg' {...register('description', { required: true })} />
                        ...
                        if (item !== 'null') {
                            return (
                                ...
                                <div key={index} className="relative">
                                    ...
                                    {item?._id ? (
                                        <img src={item?.url} alt="" className='w-[350px] h-[350px]' />
                                    ) : (
                                        <></>
                                    )}
                                </div>
