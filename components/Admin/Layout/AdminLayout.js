...
import { ChartBarIcon, AdjustmentsHorizontalIcon, ClipboardDocumentListIcon, ChevronDownIcon, PlusCircleIcon, UserGroupIcon } from '@heroicons/react/24/outline';
...
export default function AdminLayout(props) {
  ...
  const Menus = [
    { title: "Trang chủ", icon: ChartBarIcon, path: '/admin/test' },
    {
  ...
  return (
    <>
      <div className="flex">
        <div
          className={` ${open ? "w-1/6" : "w-[4%] "
            } bg-dark-purple h-screen p-5 pt-5 relative duration-200`}
        >
          <img
            src="/control.png"
                ...
                <If isTrue={Menu.children}>
                  <>
                    <div className='flex items-center p-2 mt-2 text-lg text-gray-300 rounded-md cursor-pointer hover:bg-light-white gap-x-4' onClick={() => setOpenSub(!openSub)}>
                      <Menu.icon className='w-5 h-5' />
                      <div className={`${!open && "hidden"} origin-left duration-200 flex gap-x-14`}>
                        <div className=''>
                          {Menu.title}
                        </div>
